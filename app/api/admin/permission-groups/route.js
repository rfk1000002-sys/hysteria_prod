  ...
  try {
    ...
    if (perPage < 1 || perPage > 1000) {
      return respondError({
        ...
        message: 'perPage must be between 1 and 1000',
      });
    }

    const { groups, nextCursor, hasMore } = await findAllPermissionGroups({
      perPage,
      cursor,
      search,
    });
    ...
    return NextResponse.json({
      success: true,
      data: { groups, pagination: { nextCursor, hasMore, total, perPage } },
    });
  } catch (error) {
  ...
  try {
    ...
    if (!body || !body.key) {
      return respondError({
        status: 400,
        code: 'VALIDATION_ERROR',
        message: 'PermissionGroup key is required',
      });
    }
  ...
  try {
    ...
    if (!body || !body.id) {
      return respondError({
        status: 400,
        code: 'VALIDATION_ERROR',
        message: 'PermissionGroup id is required',
      });
    }
