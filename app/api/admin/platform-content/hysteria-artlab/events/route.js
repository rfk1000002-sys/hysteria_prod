...
import { getArtlabEvents } from "@/modules/admin/platform.content/services/platform.event.content.service.js";
...
export async function GET(request) {
  try {
    ...
    const result = await getArtlabEvents(filters);

    return respondSuccess(result, 200);
  } catch (error) {
    logger.error("Error fetching hysteria artlab events", { error: error.message });
    if (error.statusCode) {
      return respondError({ message: error.message, status: error.statusCode });
    }
    return respondError({ message: "Failed to fetch artlab events", status: 500 });
  }
}
