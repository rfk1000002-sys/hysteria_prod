...
export async function POST(request) {
  try {
    ...
    if (!body.email || !body.password || !body.name) {
      logger.error('Create user failed - missing required fields', {
        adminId: user.id,
    ...
    if (!emailRegex.test(body.email)) {
      logger.error('Create user failed - invalid email format', {
        adminId: user.id,
        email: body.email,
      });
      return respondError({
    ...
    if (error.code === 'P2002') {
      logger.error('Create user failed - duplicate email', {
        error: error.message,
        stack: error.stack,
      });
      return respondError({
...
export async function PUT(request) {
  try {
    ...
    // Validate user ID
    if (!body.id) {
      logger.error('Update user failed - missing id', {
        adminId: user.id,
        body: { email: body?.email, name: body?.name },
      });
      return respondError({
    ...
    if (body.email !== undefined) {
      ...
      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!emailRegex.test(body.email)) {
        logger.error('Update user failed - invalid email format', {
          adminId: user.id,
          email: body.email,
        });
        return respondError({
    ...
    // Handle unique constraint violation (duplicate email)
    if (error.code === 'P2002') {
      logger.error('Update user failed - duplicate email', {
        error: error.message,
        stack: error.stack,
      });
      return respondError({
