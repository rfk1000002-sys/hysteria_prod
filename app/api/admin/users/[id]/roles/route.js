...
export async function GET(request, { params }) {
  try {
    ...
    if (!userId || isNaN(userId)) {
      logger.error('List user roles failed - invalid userId', {
        adminId: user.id,
...
export async function POST(request, { params }) {
  try {
    ...
    if (!userId || isNaN(userId)) {
      logger.error('Assign role failed - invalid userId', {
        adminId: user.id,
        userId: resolvedParams.id,
      });
      return respondError({
...
export async function DELETE(request, { params }) {
  try {
    ...
    if (!userId || isNaN(userId)) {
      logger.error('Remove role failed - invalid userId', {
        adminId: user.id,
        userId: resolvedParams.id,
      });
      return respondError({
...
export async function PUT(request, { params }) {
  try {
    ...
    if (!userId || isNaN(userId)) {
      logger.error('Replace roles failed - invalid userId', {
        adminId: user.id,
        userId: resolvedParams.id,
      });
      return respondError({
