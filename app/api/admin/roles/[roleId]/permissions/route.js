...
export async function POST(request, { params }) {
  try {
    ...
    if (!permissionId) {
      logger.error('Assign permission failed - missing permissionId', {
        adminId: user.id,
        roleId,
        body,
      });
    ...
    if (!perm) {
      logger.error('Assign permission failed - permission not found', {
        adminId: user.id,
        roleId,
        permissionId,
      });
...
export async function PUT(request, { params }) {
  ...
  try {
    ...
    if (!Array.isArray(permissionIds)) {
      logger.error('Replace permissions failed - invalid permissionIds', {
        adminId: user.id,
        roleId,
        body,
      });
    ...
    logger.info('Replaced permissions for role', {
      adminId: user.id,
      roleId,
