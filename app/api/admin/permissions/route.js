...
export async function GET(request) {
  try {
    ...
    const { permissions, nextCursor, hasMore } = await findAllPermissions({
      perPage,
      cursor,
      search,
      groupId,
    ...
    return NextResponse.json({
      success: true,
      data: { permissions, pagination: { nextCursor, hasMore, total, perPage } },
    });
  } catch (error) {
  ...
  try {
    ...
    if (Array.isArray(body)) {
      ...
      for (const item of body) {
        if (!item || !item.key) {
          logger.error('Create permissions failed - missing key in item', {
            adminId: user.id,
            item,
          });
          return respondError({
            status: 400,
            code: 'VALIDATION_ERROR',
            message: 'Each permission must include a key',
          });
        }
      }
