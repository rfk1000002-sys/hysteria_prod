...
export default function ForumPage() {
  ...
  return (
    <main className="min-h-screen bg-white text-black font-sans pb-20">
      ...
      {/* 2. CONTENT SECTION */}
      {/* - max-w-[1440px]: Sesuai ukuran laptop full.
          - px-6 md:px-16: Padding lebar agar sesuai dengan desain "image_58a5db.png"
      */}
      <div className="w-full max-w-[1440px] mx-auto px-6 md:px-16 py-12">
        
        {/* FILTER TABS (DESAIN BARU SESUAI GAMBAR) */}
        {/* Menggunakan justify-between atau gap lebar agar menyebar seperti di desain */}
          ...
          {KATEGORI.map((cat) => (
            <button
              ...
              className={`relative pb-3 text-base md:text-lg transition-all duration-300
                ${activeTab === cat
                  ? "text-black font-extrabold" // Aktif: Hitam Tebal
                  : "text-black font-medium hover:text-[#D63384]" // Tidak Aktif: Hitam Medium
                }`}
            >
              {cat}
              {/* Garis Bawah Pink Tebal & Membulat (Hanya muncul di tab aktif) */}
              {activeTab === cat && (
          ...
          {filteredData.map((post) => (
            ...
            >
              ...
              {/* --- 2. DEFAULT OVERLAY (ORANGE TINT) --- 
                  Efek poster oranye saat diam
              */}
              <div className="absolute inset-0 bg-orange-500/40 mix-blend-multiply transition-opacity duration-300 group-hover:opacity-0" />

              {/* --- 3. HOVER OVERLAY (DARK) --- 
                  Efek gelap saat dihover
              */}
              <div className="absolute inset-0 bg-black/60 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-all duration-300" />
              ...
              <div className="absolute inset-0 p-6 flex flex-col justify-end">
                ...
                {/* HOVER VIEW: Detail Lengkap (Muncul saat hover) */}
                <div className="flex flex-col justify-end items-start transform translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 delay-75 w-full">
                    ...
                    {/* Badge Status (Putih text Pink) */}
                    <div className="mb-3">
                       <span className="bg-white text-[#D63384] text-[10px] font-bold px-3 py-1 rounded-full shadow-sm uppercase tracking-wide">
                          {post.status}
                       </span>
                    </div>
