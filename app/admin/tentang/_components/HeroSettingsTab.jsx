...
export default function HeroSettingsTab() {
  ...
  const fetchHero = useCallback(async () => {
    ...
    try {
      ...
      if (!json?.success) {
        ...
        throw new Error(json?.error?.message || "Gagal memuat hero tentang");
      }
      ...
      setToast({ visible: true, message: error?.message || "Gagal memuat hero", type: "error" });
    } finally {
      setLoading(false);
    }
  }, [apiCall]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const json = await res.json().catch(() => null);
      if (!json?.success) throw new Error(json?.error?.message || "Gagal menyimpan hero");
      ...
      setClearImage(false);
      setToast({ visible: true, message: "Hero tentang berhasil disimpan", type: "success" });
    } catch (error) {
      setToast({ visible: true, message: error?.message || "Gagal menyimpan hero", type: "error" });
    } finally {
      setSaving(false);
    }
  };
    ...
    <PermissionGate requiredPermissions={["tentang.read"]}>
      <div className="space-y-4">
        <Card
          ...
          subtitle="Atur hero untuk halaman tentang (slug: tentang).">
          {loading ? (
