...
export default function PermissionsTab() {
  ...
  const fetchPermissions = useCallback(
    async (cursor = null, searchTerm = '', groupId = null) => {
      try {
        setLoading(true);
  ...
  const fetchGroups = useCallback(async () => {
    try {
      ...
      const json = await res.json();
      const items =
        (json?.data && (json.data.permissionGroups || json.data.groups || json.data)) || [];
      setGroups(items);
    } catch (err) {
      console.error(err);
  ...
  const handleCreateSubmit = async (e) => {
    ...
    try {
      ...
      setCreateOpen(false);
      setToast({
        message: `${createdItems.length} permission(s) created`,
        type: 'info',
        visible: true,
      });
    } catch (err) {
  ...
  const startEditWithGroup = (p) =>
    setEditing({ id: p.id, key: p.key, name: p.name || '', groupId: p.group ? p.group.id : null });
        ...
        <div className="flex items-center gap-2">
          ...
          <PermissionGate requiredPermissions={'permissions.delete'} disableOnDenied>
            <IconButton
              ...
              size="small"
              onClick={() => openDeleteConfirm(r.id, r.key || r.name)}
      ...
      {editing && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white p-4 sm:p-6 rounded-md w-full max-w-xl">
            <h3 className="text-base sm:text-lg font-medium mb-3 sm:mb-4 text-zinc-900">
              Edit Permission
            </h3>
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button
                ...
                className="px-4 py-2 bg-zinc-200 text-zinc-900 rounded-md hover:bg-zinc-300 focus:outline-none focus:ring-2 focus:ring-zinc-300"
              >
                Cancel
              </button>
            ...
            <form onSubmit={handleCreateSubmit} className="flex flex-col gap-3">
              <div className="flex flex-col gap-3">
                {newItems.map((it, idx) => (
                  <div
                    key={idx}
                    className="flex flex-col sm:grid sm:grid-cols-12 gap-2 sm:items-end border-b sm:border-0 pb-3 sm:pb-0"
                  >
                ...
                <div className="flex items-center gap-2">
                  <button type="button" onClick={addNewRow} className="text-sm text-blue-600">
                    + Add row
                  </button>
                </div>
              </div>
