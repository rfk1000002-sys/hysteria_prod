...
export default function UserManagement() {
  ...
  const openRoleModal = async (user) => {
    ...
    if (isSuperAdminUser(user)) {
      setToast({
        message: 'Cannot change roles for Super Admin users',
        type: 'error',
        visible: true,
      });
      return;
    ...
    try {
      ...
      const filteredAvailable = allAvailable.filter(
        (r) => normalizeKey(r.key || r.role?.key) !== 'SUPERADMIN'
      );
  ...
  const openStatusModal = async (user) => {
    ...
    if (isSuperAdminUser(user)) {
      setToast({
        message: 'Cannot change status for Super Admin users',
        type: 'error',
        visible: true,
      });
      return;
        ...
        <div className="grid grid-cols-2 gap-1 justify-items-center">
            ...
            {(() => {
              ...
              return (
                <IconButton
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    if (!cannotEditSuper) openEditModal(r);
                    else
                      setToast({
                        message: 'Only Super Admin can edit Super Admin users',
                        type: 'error',
                  ...
                  title={
                    cannotEditSuper ? 'Only Super Admin can edit Super Admin users' : 'Edit user'
                  }
          ...
          <PermissionGate requiredPermissions={'users.delete'} disableOnDenied>
            <IconButton
              ...
              onClick={(e) => {
                e.stopPropagation();
                if (!isSuperAdminUser(r)) handleDelete(r);
              }}
      ...
      content: ({ onClose }) => (
        <div className="space-y-4 text-sm text-zinc-800">
          <div className="space-y-2">
            ...
            <div>
              <strong>Status:</strong> {row.status?.name || '-'}
            </div>
            <div>
            ...
            <div>
              <strong>Roles:</strong>{' '}
              {row.roles?.length ? row.roles.map((r) => r.role?.key).join(', ') : 'No roles'}
            </div>
          </div>
