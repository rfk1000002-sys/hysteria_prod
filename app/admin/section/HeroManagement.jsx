...
export default function HeroManagement() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const mediaFile =
        formData.media &&
        (formData.media instanceof File ? formData.media : formData.media[0] || null);
      if (mediaFile) {
        // enforce configured max size
        if (mediaFile.size && mediaFile.size > MAX_MEDIA_SIZE) {
          setToast({
            visible: true,
            message: `File size must be ${MAX_MEDIA_SIZE_MB} MB or less`,
            type: 'error',
          });
          return;
        }
  ...
  function validateForm(data, mode = 'create') {
    ...
    if ((!data.source || String(data.source).trim() === '') && !data.media)
      errs.push('Masukkan URL atau unggah file');
    ...
    if (data.source && String(data.source).trim() !== '') {
      ...
      try {
        ...
        const looksLikeLocal =
          src.startsWith('/') || src.startsWith('uploads/') || !src.includes(':');
        if (!looksLikeLocal) {
  ...
  const columns = [
...

    {
      ...
      headerName: 'Title',
      freeze: true,
      render: (hero) => <div className="font-medium text-zinc-900">{hero.title}</div>,
    },
      ...
      <Card>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-3">
          <div className="min-w-0">
            <h2 className="text-xl font-semibold text-zinc-900 truncate">Hero Management</h2>
            <p className="mt-1 text-sm text-zinc-500 truncate">Manage heroes for your homepage</p>
        ...
        <DataTable rows={heroes} columns={columns} loading={loading} getRowId={(row) => row.id} />
      </Card>
      ...
      <CrudModals
        ...
        onSubmit={async (data) => {
          ...
          try {
            ...
            const mediaFile =
              data.media && (data.media instanceof File ? data.media : data.media[0] || null);
            if (mediaFile) {
              // enforce configured max size
              if (mediaFile.size && mediaFile.size > MAX_MEDIA_SIZE) {
