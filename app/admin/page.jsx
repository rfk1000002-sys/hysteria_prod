    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">
              Berbagai contoh penggunaan komponen Card yang dinamis
            </p>
          </div>
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card size="sm" variant="elevated" hoverable>
          <CardStats label="Total Users" value="2,543" trend="+12.5%" trendDirection="up" />
        </Card>
        <Card size="sm" variant="elevated" hoverable>
          <CardStats label="Revenue" value="$45,231" trend="+8.2%" trendDirection="up" />
        </Card>
        <Card size="sm" variant="elevated" hoverable>
          <CardStats label="Active Sessions" value="142" trend="-3.1%" trendDirection="down" />
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card
          ...
          actions={
            <>
              <button className="px-3 py-1 text-sm border border-zinc-200 rounded hover:bg-zinc-50">
                Edit
              </button>
        ...
        >
          <div className="space-y-2">
            ...
            <p className="text-sm">
              <strong>Status:</strong> <CardBadge variant="success">Active</CardBadge>
            </p>
          </div>
        </Card>

        <Card title="Recent Activity" collapsible defaultCollapsed={false} variant="elevated">
      ...
      {/* Clickable Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card hoverable onClick={() => alert('Card 1 clicked!')} title="Clickable Card">
          <p className="text-sm text-zinc-600">Click me untuk melihat alert!</p>
        </Card>

        <Card hoverable variant="outlined" title="Documentation">
          <p className="text-sm text-zinc-600">Hover untuk melihat efek shadow.</p>
        </Card>

        <Card loading={false} title="Loading Example">
          <p className="text-sm text-zinc-600">
            Ubah loading prop menjadi true untuk lihat spinner.
          </p>
        </Card>
      </div>
      ...
      <div>
        <h3 className="text-lg font-semibold text-zinc-900 mb-4">
          Social Media Links (Dynamic Data)
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            {
      ...
      <div className="relative p-8 rounded-3xl overflow-hidden">
        ...
        <div
          className="absolute top-0 right-1/4 w-96 h-96 bg-purple-500 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-pulse"
          style={{ animationDelay: '2s' }}
        ></div>
        <div
          className="absolute -bottom-8 left-1/3 w-96 h-96 bg-pink-500 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-pulse"
            ...
            {[
              ...
              >
                <div className="space-y-4">
                  ...
                  <ul className="space-y-2">
                    {plan.features.map((feature, idx) => (
                      <li key={idx} className="flex items-center gap-2 text-sm text-white/90">
                        <svg
                          className="w-5 h-5 text-emerald-300 flex-shrink-0 drop-shadow-md"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
                  ...
                  <button className="w-full px-4 py-2 bg-white/20 hover:bg-white/30 backdrop-blur-md text-white rounded-lg font-medium transition-all border border-white/30 hover:border-white/50 shadow-lg hover:shadow-xl"></button>
                </div>
              </Card>
            ))}
          </div>
