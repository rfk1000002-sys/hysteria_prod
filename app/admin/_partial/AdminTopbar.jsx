...
export default function AdminTopbar({ onOpenSidebar }) {
  ...
  const headerTitle =
    user?.roles && user.roles.length > 0
      ? `${formatRoleLabel(user.roles[0])} Area`
      : 'Dashboard Panel';
  return (
    <div className="flex w-full items-center justify-between px-6 py-4">
      ...
      <button
        ...
        className="flex items-center justify-center rounded-full bg-zinc-50 h-10 w-10 hover:bg-zinc-100 focus:outline-none focus:ring-0"
      >
        <Avatar
