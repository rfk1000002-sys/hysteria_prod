...
export default function CrudModals({
  ...
  const requiredMissing = useMemo(() => {
    return fields.some((fld) => {
      ...
      if (fld.name === 'media' && form && form.source && String(form.source).trim() !== '')
        return false;
  ...
  function FilePreview({ value, name }) {
    ...
    useEffect(() => {
      ...
      if (
        typeof value === 'string' &&
        (value.startsWith('/') || value.startsWith('https') || value.startsWith('uploads/'))
      ) {
        setPreviewUrl(value);
        return;
      }
    ...
    const size =
      typeof value === 'string' ? null : typeof value.size === 'number' ? value.size : null;
    ...
    return (
      <div className="mt-2 flex items-center gap-3 border border-zinc-300 bg-white rounded-md p-3 shadow-sm">
        {previewUrl ? (
          ...
          <div className="w-16 h-16 rounded-md bg-zinc-50 flex items-center justify-center text-zinc-500 text-sm border border-zinc-200">
            FILE
          </div>
        )}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-1 text-xs text-zinc-500">
            {typeof value === 'string' ? 'Path tersimpan' : value.type || ''}
          </div>
        </div>
        ...
        <button
          type="button"
          onClick={remove}
          className="ml-2 inline-flex items-center gap-2 rounded-md px-2 py-1 text-sm font-medium text-red-600 hover:bg-red-50"
        >
            ...
            <div key={fld.name}>
              ...
              {fld.type === 'textarea' ? (
                ...
                <div>
                  ...
                  {form[fld.name] ? <FilePreview value={form[fld.name]} name={fld.name} /> : null}
                </div>
              ) : (
                <input
                  type={fld.type || 'text'}
              ...
              {fld.validation ? (
                <p className="mt-1 text-xs text-red-600">{fld.validation}</p>
              ) : null}
  ...
  const footer = ({ onClose: shellClose }) => (
    <div className="flex justify-end gap-3 pt-4 border-t border-zinc-200 w-full">
      ...
      <button
        ...
        disabled={loading || (isDelete ? false : requiredMissing || (mode === 'edit' && !isDirty))}
  ...
  return (
    <DialogShell
      opts={{ title: modalTitle, content, footer, size }}
      onClose={onClose}
      onConfirm={() => {
        /* no-op */
      }}
