...
const ABOUT_MENU_MOBILE_ITEMS = [
  { href: "/tentang", label: "Tentang Hysteria" },
  { href: "/tim", label: "Tim Hysteria" },
];
    ...
    <li className="w-full">
      <div className="flex items-center gap-2 justify-end w-full text-xl font-semibold text-right">
        <button
          ...
          <ArrowIcon
            active={subOpen && isActive}
            rotate={subOpen && isActive}
          />
        </button>
        <button
          onClick={(e) => {
 ...
 */
function DesktopSubmenuPanel({ activeSub, categoryCache, teamCache, loading, error, handleClose }) {
  const getTitle = () => {
    const titles = {
          ...
          {activeSub === "about" && (
            <>
              <div className="mb-4">
                ...
                <ul
                  id="submenu-about"
                  className="space-y-1 text-right pr-2">
                  {ABOUT_MENU_ITEMS.map(({ href, label }) => (
                    <li key={href}>
                      <Link
                        href={href}
                        onClick={handleClose}
                        className="text-gray-700 cursor-pointer hover:text-[#E83C91]">
                        {label}
                      </Link>
                    </li>
                  ))}
...
export default function MobileMenu({ open, onClose }) {
  ...
  /**
   * Load team-member category data from API (cached separately)
   */
  async function loadTeamCategory(key) {
    setLoadingTeam(true);
    setTeamError(null);
    ...
    try {
      ...
      const arr = categories.map((cat) => ({
        slug: cat.slug || String(cat.id) || cat.name,
        title: cat.name || "",
      }));
      ...
      setTeamError("Gagal memuat kategori tim");
      setTeamCache([]);
    } finally {
  ...
  function openSub(key) {
    ...
    if (["program", "platform", "artikel", "about"].includes(key)) {
      loadCategory(key);
      loadTeamCategory(key);
    } else {
      setActiveSub(key);
      setSubOpen(true);
    }
                ...
                <li className="w-full">
                  ...
                  {isMobile && subOpen && activeSub === "about" && (
                    <div className="pr-4 mt-2">
                      <div className="border-t border-b border-gray-200 p-0">
                        <ul
                          id="submenu-about-mobile"
                          className="space-y-1 text-right">
                          {ABOUT_MENU_MOBILE_ITEMS.map(({ href, label }) => (
                            <li key={href}>
                              <Link
                                href={href}
                                onClick={handleClose}
