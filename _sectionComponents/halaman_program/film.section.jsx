...
export default function FilmSection() {
  ...
  useEffect(() => {
    async function fetchFilmEvents() {
      try {
        const res = await fetch('/api/admin/programs'); 
        if (!res.ok) throw new Error("Gagal mengambil data");
        const allPrograms = await res.json();
        
        const filmEvents = allPrograms
          .filter(event => 
            event.isPublished && 
            event.eventCategories?.some(cat => filmSlugs.includes(cat.categoryItem?.slug))
          )
          .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
          .slice(0, 10); 

        setFilmData(filmEvents);
      } catch (error) {
  ...
  const handleScroll = () => {
    if (scrollRef.current && scrollRef.current.children.length > 0) {
      ...
      if (Math.abs(maxScrollLeft - scrollPos) <= 5) {
        setActiveIndex(filmData.length - 1);
      } else {
        const firstCard = container.children[0];
        const itemWidth = firstCard ? firstCard.clientWidth : 300; 
        // Gap disesuaikan dengan responsif (16px di mobile, 24px di desktop)
        const gap = window.innerWidth < 768 ? 16 : 24; 
        const totalItemWidth = itemWidth + gap; 
        const index = Math.round(scrollPos / totalItemWidth);
        
        setActiveIndex(Math.min(index, filmData.length - 1));
      }
    }
  };
  ...
  if (isLoading) {
    return (
      <section className="w-full max-w-[1440px] mx-auto px-6 md:px-10 lg:px-20 mb-16 md:mb-20 flex justify-center py-10 md:py-20">
         <div className="animate-pulse text-[#E83C91] font-bold">Memuat Data Pemutaran Film...</div>
      </section>
    );
  }
    ...
    return (
      <section className="w-full max-w-[1440px] mx-auto px-6 md:px-10 lg:px-20 mb-16 md:mb-20">
        <div className="flex justify-between items-center mb-6 md:mb-8">
          <h2 className={`${poppins.className} text-[22px] md:text-[32px] font-bold text-black`}>Pemutaran Film</h2>
        </div>
        <div className="w-full py-10 flex justify-center items-center bg-gray-50 border-2 border-dashed border-gray-200 rounded-xl">
           <p className="text-gray-400 font-medium text-sm md:text-base">Belum ada acara Pemutaran Film yang diterbitkan.</p>
        </div>
      </section>
    );
  }
  ...
  return (
    <section id="pemutaran-film" className="w-full max-w-[1440px] mx-auto px-6 md:px-10 lg:px-20 mb-16 md:mb-20">
      ...
      <div className="flex justify-between items-center mb-6 md:mb-8">
        ...
        <Link href="/program/pemutaran-film" className="text-xs md:text-sm text-black font-semibold underline underline-offset-4 hover:text-[#D63384] transition">
            Lihat Semua
        </Link>
      </div>
      ...
      >
        {filmData.map((post) => {
          ...
          const specificCategory = post.eventCategories?.find(cat => filmSlugs.includes(cat.categoryItem?.slug))?.categoryItem?.title || "Film";
          ...
          return (
            ...
            >
              ...
              {/* Tampilan Default (Selalu tampil di Mobile) */}
              ...
              {/* Tampilan Saat Di-Hover (Hanya aktif di Desktop) */}
              <div className="absolute inset-0 z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 hidden md:block">
                  ...
                  <div className="relative z-10 h-full p-6 flex flex-col justify-end items-start text-left">
                      ...
                      <Link 
                          href={`/program/pemutaran-film/${post.id}`}
              ...
              <Link href={`/program/pemutaran-film/${post.id}`} className="absolute inset-0 z-30 md:hidden"></Link>
            </div>
          );
        })}
      </div>
