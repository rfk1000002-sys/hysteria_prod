...
const CustomPrevArrow = ({ onClick }) => (
  <button
    onClick={onClick}
    className="hidden sm:block absolute -left-2.5 md:-left-5 top-1/2 -translate-y-1/2 z-[2] bg-white/10 backdrop-blur-[10px] border border-white/20 w-[35px] md:w-[45px] h-[35px] md:h-[45px] hover:bg-white/20 text-white rounded-full transition-all"
  >
...
const CustomNextArrow = ({ onClick }) => (
  <button
    onClick={onClick}
    className="hidden sm:block absolute -right-2.5 md:-right-5 top-1/2 -translate-y-1/2 z-[2] bg-white/10 backdrop-blur-[10px] border border-white/20 w-[35px] md:w-[45px] h-[35px] md:h-[45px] hover:bg-white/20 text-white rounded-full transition-all"
  >
    <svg className="w-6 h-6 md:w-7 md:h-7 mx-auto" fill="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <section className="bg-[#FFFFFF] py-8 md:py-20 mt-20 relative overflow-visible"> {/* allow cards to move outside without being clipped */}
      <div className="max-w-[1920px] w-full mx-auto px-2 sm:px-2">
        ...
        <div className="relative px-2 md:px-7 pt-2 overflow-visible rounded-xl" ref={containerRef}>
            ...
            {dummyEvents.map((event) => (
              ...
              <div key={event.id} className="px-2 md:px-1.5 pt-4 pb-4 mt-0 md:mt-0">
                ...
                >
                  ...
                  <div className={`event-overlay absolute inset-0 z-10 bg-gradient-to-t from-black/95 via-black/80 to-transparent p-2 sm:p-3 transition-opacity duration-300 ease-in-out flex flex-col justify-end ${activeId === event.id ? "opacity-100" : "opacity-0"} group-hover:opacity-100`}>
                    <h6 className="text-white font-bold text-sm md:text-[1.1rem] mb-1 leading-tight">
                      {event.title}
                    </h6>
                    <p className="text-white/80 text-[0.72rem] md:text-[0.85rem] mb-0.5">
                      📅 {event.date}
                    </p>
                    <p className="text-white/80 text-[0.72rem] md:text-[0.85rem] mb-2">
                      📍 {event.location}
                    </p>
                    <button onClick={(e)=>e.stopPropagation()} className="bg-[#7c3aed] text-white font-semibold text-xs sm:text-sm py-1 rounded-lg hover:bg-[#6d28d9] transition-colors">
                      Ikuti Sekarang
                    </button>
                  </div>
                </div>
