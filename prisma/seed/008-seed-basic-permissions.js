...
module.exports = async function seed() {
	...
	try {
       ...
       RETURNING id`,
		...
		if (!permGroupId) {
			const existing = await client.query(`SELECT id FROM "PermissionGroup" WHERE key = $1`, [
		...
		const permissions = [
			// User permissions
			{ key: 'users.read', name: 'Read Users', description: 'View user list and details', groupId: userGroupId },
			{ key: 'users.create', name: 'Create Users', description: 'Create new users', groupId: userGroupId },
			{ key: 'users.update', name: 'Update Users', description: 'Update existing users', groupId: userGroupId },
			{ key: 'users.delete', name: 'Delete Users', description: 'Delete users', groupId: userGroupId },
