  ...
  if (response.status === 401 && !url.includes('/api/auth/')) {
    ...
    if (errorData?.code === 'INVALID_TOKEN_VERSION') {
      ...
      if (typeof window !== 'undefined') {
        // Persist message so it survives the navigation and notify listeners
        try {
        ...
        // Delay logout sedikit agar notification sempat ditampilkan
        setTimeout(() => {
    ...
    try {
      ...
      // Jika refresh gagal, notify client and redirect ke login
      if (typeof window !== 'undefined') {
        const msg = 'Terjadi kesalahan. Silakan login kembali.'
        try {
          sessionStorage.setItem('auth:logoutMessage', msg)
        } catch (e) {
          // ignore
        }
        try {
