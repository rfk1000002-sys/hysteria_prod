...
export async function createWithUpload(
  { createRecord, uploadFile, updateSource, deleteRecord },
  file
) {
  ...
  try {
    ...
    logger.error('createWithUpload: DB insert failed', {
      error: dbErr && (dbErr.stack || dbErr.message),
    });
    throw dbErr;
  }
  ...
  try {
    ...
    try {
      if (created && created.id != null) {
        ...
        logger.info('createWithUpload: Successfully rolled back DB record after upload failure', {
          recordId: created.id,
        });
      }
    } catch (delErr) {
  ...
  try {
    ...
    logger.info('createWithUpload: Successfully created record with uploaded file', {
      recordId: created.id,
 ...
 */
export async function updateWithUpload(
  { getExisting, updateRecord, uploadFile, updateSource, revertRecord, deleteFile },
  id,
  data,
  file
) {
  ...
  try {
    ...
    logger.error('updateWithUpload: Failed to get existing record', {
  ...
  try {
    ...
    logger.error('updateWithUpload: DB update failed', {
      error: dbErr && (dbErr.stack || dbErr.message),
      id,
    });
    throw dbErr;
  ...
  try {
    ...
    if (existing.source && existing.source !== uploadResult.url) {
      try {
        ...
        logger.info('updateWithUpload: Successfully deleted old file', {
    ...
    logger.info('updateWithUpload: Successfully updated record with new file', {
      id,
      url: uploadResult.url,
    });
    return final;
