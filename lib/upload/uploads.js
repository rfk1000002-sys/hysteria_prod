...
function genFileName(originalName, extOverride = '') {
  const ext =
    ...
    `.${mime.extension(mime.lookup(originalName) || 'bin')}`;
...
export default class Uploads {
  constructor() {
    ...
    this.storage = (
      process.env.UPLOAD_STORAGE || (process.env.S3_BUCKET ? 's3' : 'local')
    ).toLowerCase();
    ...
    if (this.storage === 's3' && !this.useS3) {
      ...
      console.warn(
        'UPLOADS: UPLOAD_STORAGE=s3 requested but S3_BUCKET missing — falling back to local storage.'
      );
      this.storage = 'local';
      this.useS3 = false;
    }
  ...
  async deleteFile(source) {
    ...
    if (this.useS3) {
      let key = source;
      if (process.env.S3_PUBLIC_URL && source.startsWith(process.env.S3_PUBLIC_URL)) {
        key = source.replace(process.env.S3_PUBLIC_URL.replace(/\/$/, '') + '/', '');
      } else {
        // If it's a full S3 URL like https://bucket.s3.region.amazonaws.com/key
        const s3Match = source.match(/https?:\/\/[^/]+\/(.+)$/);
