...
export async function assignPermissionToRole(roleId, permissionId) {
  const rp = await prisma.rolePermission.create({
    data: { roleId: parseInt(roleId), permissionId: parseInt(permissionId) },
  });
  ...
  try {
    ...
    console.warn(
      "Failed to bump tokenVersion or revoke tokens after assignPermissionToRole",
      e && e.message,
    );
  }
...
export async function removePermissionFromRole(roleId, permissionId) {
  const res = await prisma.rolePermission.deleteMany({
    where: { roleId: parseInt(roleId), permissionId: parseInt(permissionId) },
  });
  ...
  try {
    ...
    console.warn(
      "Failed to bump tokenVersion or revoke tokens after removePermissionFromRole",
      e && e.message,
    );
  }
...
export async function replacePermissionsForRole(roleId, permissionIds = []) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (!permissionIds || permissionIds.length === 0) {
      ...
      await tx.user.updateMany({
        where: { roles: { some: { roleId: rid } } },
        data: { tokenVersion: { increment: 1 } },
      });
      return [];
    }
    const creates = permissionIds.map((pid) => ({
      roleId: rid,
  ...
  try {
    ...
    console.warn(
      "Failed to bump tokenVersion or revoke tokens after replacePermissionsForRole",
      e && e.message,
    );
  }
